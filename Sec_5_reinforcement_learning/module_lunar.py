####################################################################################################
# THIS REPO CONTAINS SLIGHLTY MODIFIED VERSION OF THE ORIGINAL GYMNASIUM CODE                      #
...
INITIAL_RANDOM = 100.0  # Set 1500 to make game harder
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        # Create Terrain
        CHUNKS = 11
        height = self.np_random.uniform(0, H / 4, size=(CHUNKS + 1,))#height = self.np_random.uniform(0, H / 2, size=(CHUNKS + 1,))
        ...
        smooth_y = [0.6670, 1.0359, 1.9973, 2.5812, 3.3000, 3.300, 3.3000, 2.9905, 2.0819, 1.4213, 0.6432]
...
class RewardFN():
    ...
    def reward(self, state, a):
        ...
        if abs(state[0]) >= 1.0:
            ...
            # If the acceleration suggests that the lander will crash in the next frame, then the reward is -100
            if state[1] + state[3]/FPS < -0.05:
                reward = -100.0
                self.perpetual_reward = reward
            elif state[1] <= -0.03:
                reward = -100.0
                self.perpetual_reward = reward
            elif state[1] <= 0.01 and np.abs(state[2]) <= 0.0001 and np.abs(state[3]) <= 0.0001 and np.abs(state[0]) <= 0.1:
                # If the lander is close to the ground and has a low velocity in both the x and y directions, we consider it landed, and the reward is 100
                reward = 100.0
                self.perpetual_reward = reward

        if self.perpetual_reward is not None:
            reward = self.perpetual_reward
        return reward
